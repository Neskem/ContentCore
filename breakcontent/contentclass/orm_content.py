...
from breakcontent.model.content import UrlInfo, TaskInfo, ServiceInfo
import datetime
...
def init_sitemap_url_info(url, url_hash, partner_id, domain):
    new_init_partner = UrlInfo(
        url=url, url_hash=url_hash, partner_id=partner_id, domain=domain)
    pg_add_wrapper(new_init_partner)
...
def init_sitemap_service_info(url_hash, service, status):
    new_init_service = ServiceInfo(
        url_hash=url_hash, service=service, status=status)
    pg_add_wrapper(new_init_service)
...
def exist_url_hash_serviceinfo(url_hash):
    url_hash_res_serviceinfo = ServiceInfo.query.filter_by(
...
def exist_finished_taskinfo(url_hash):
    url_hash_finished_res = TaskInfo.query.filter_by(
        url_hash=url_hash, notify_status='finished').first()
    url_hash_pending_res = TaskInfo.query.filter_by(
        url_hash=url_hash, notify_status='pending').first()
...
def check_task_status_to_doing(url_hash):
    check_task_res = TaskInfo.query.filter_by(
        url_hash=url_hash, notify_status='pending').first()
    if check_task_res:
