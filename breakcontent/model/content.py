...
class UrlInfo(db.Model):
    ...
    url_hash = db.Column(db.String(256), nullable=True,
    ...
    has_page_code = db.Column(db.Boolean, nullable=True)
...
class TaskInfo(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    url_hash = db.Column(db.String(64), db.ForeignKey(
        'url_info.url_hash'), nullable=False, index=True)
    # notify_status: pending, doing(already send task to content core), finished, failed(content core disconnection)
    notify_status = db.Column(db.String(64), nullable=True)
    generator = db.Column(db.String(256), nullable=True)
    sync_priority = db.Column(db.Integer, nullable=True)
    async_priority = db.Column(db.Integer, nullable=True)
    others_priority = db.Column(db.Integer, nullable=True)
...
class ServiceInfo(db.Model):
    ...
    url_hash = db.Column(db.String(64), db.ForeignKey(
        'url_info.url_hash'), nullable=False, index=True)
    partner_id = db.Column(db.String(64), nullable=True)
    status = db.Column(db.Enum('sync', 'syncing', 'remove',
                               'failed', 'finished', name='sync_status'), nullable=True)
