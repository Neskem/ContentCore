...
def dpi_callback2():
    ...
    While watch app-content.log that before sleep fun the result.ready() is False,
        but after sleep fun the result.ready() is True.

    Print content-worker.log will see "Alan_test: test_delay" output from tasks.py
    ...
    from breakcontent.tasks import test_delay
...
def dpi_callback3():
    ...
    Add content to config.py:
        CELERY_ROUTES = {
            'breakcontent.tasks.test_queue': {'queue': 'test_queue'},
        }

    Execute worker in content-dev container:
    /usr/bin/python3 /usr/local/bin/celery -A breakcontent.tasks worker -Q test_queue -l info &
    ...
    from breakcontent.tasks import test_queue
